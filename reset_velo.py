...
def main():
    ...
    # 1. Wipe all existing motos, locacoes, and velo transactions
    cursor.execute("DELETE FROM locacoes")
    cursor.execute("DELETE FROM motos")
    cursor.execute("DELETE FROM transacoes WHERE origem = 'VELO'")
    print("Dados antigos de motos, locações e transações Velo foram apagados.")
    ...
    # 3. Create a single 'Cliente Único Velo'
    ...
    print("Locações ativas vinculadas ao contrato Velo.")

    # 4. Insert Receitas (Entradas Velo)
    receitas = [
    ...
    # 5. Insert Despesas (Saídas Velo)
    despesas = [
        ("VELO", "saida", 74605.88, data_insercao, None, None), # Compra de Moto
    ...
    conn.commit()
    conn.close()
    print("Database reiniciada com sucesso. Apenas as 5 motos reais da Velo e suas transações constam no sistema.")
